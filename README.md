  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      ...
      --bg:#f4f1f9;--card:#ffffff;--card2:#ede8f5;--card3:#e4ddf0;
      --text:#1a0e2e;--muted:#6b5a88;--muted2:#9b8aaa;
      --border:rgba(46,26,71,.1);--border2:rgba(212,168,83,.2);
      --green:#16a34a;--red:#dc2626;
      --r:20px;--rs:14px;
      --shadow:0 4px 16px rgba(46,26,71,.08);
      --shadow-gold:0 4px 16px rgba(212,168,83,.2);
    }
    [data-theme="dark"]{
    ...
    #screen-loader{display:none;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;background:var(--p)}
    #screen-loader.active{display:flex !important}
    ...
    /* LOGIN */
    #screen-login{display:none;background:linear-gradient(160deg,var(--p),var(--p3));min-height:100vh;flex-direction:column;padding-bottom:0}
    ...
    /* TOPBAR — MEMBRO */
    .topbar{background:var(--card);backdrop-filter:blur(20px);box-shadow:0 2px 12px var(--shadow);padding:.9rem 1.25rem;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:10;border-bottom:1px solid var(--border)}
    ...
    .topbar-title{color:var(--p2);font-size:15px;font-weight:700;flex:1}
    .topbar-back,.topbar-icon{background:var(--card2);border:none;border-radius:10px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:17px;color:var(--p2);flex-shrink:0}

    /* TOPBAR — ADMIN */
    .topbar.admin-bar{background:var(--p);border-bottom:1px solid var(--border2)}
    ...
    .tx-item{display:flex;align-items:center;gap:12px;padding:11px 14px;margin-bottom:8px;background:var(--card);border-radius:16px;border:1px solid var(--border);box-shadow:var(--shadow)}
    ...
    .modal{background:var(--card);border-radius:24px 24px 0 0;padding:2rem 1.5rem;width:100%;max-width:420px;box-shadow:0 -8px 40px var(--shadow);border-top:1px solid var(--border)}
    ...
    /* BOTTOM NAV */
    .bottom-nav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:420px;background:var(--card);backdrop-filter:blur(20px);box-shadow:0 -2px 20px var(--shadow);border-top:1px solid var(--border);display:none;padding:.55rem 0 .8rem;z-index:20}
...
<div id="screen-settings" class="screen">
  ...
  <div style="padding-top:.5rem">
    ...
    <div class="menu-item" onclick="toggleTheme()">
      ...
      <div class="menu-label" id="theme-menu-label">mudar para dark mode</div>
      <div style="margin-left:auto;background:var(--card2);border-radius:20px;padding:3px;display:flex;gap:2px">
        <div id="theme-toggle-knob" style="width:24px;height:24px;border-radius:18px;background:var(--p2);transition:transform .2s"></div>
      </div>
    </div>
...
function applyTheme(theme) {
  ...
  localStorage.setItem('dracmas-theme', theme);
  ...
  if (label) label.textContent = isDark ? 'mudar para light mode' : 'mudar para dark mode';
...
function initTheme() {
  const saved = localStorage.getItem('dracmas-theme') || 'light';
  applyTheme(saved);
}
